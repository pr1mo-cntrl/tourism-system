    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                // --- SECTION 1: ESTABLISHMENT PROFILE ---
                \Filament\Forms\Components\Fieldset::make('Establishment Profile')
                    ->schema([
                        Select::make('name') 
                            ->label('Name of Establishment')
                            ->columnSpanFull() // Makes the name span across the whole card
                            ->searchable()
                            ->live()
                        ...
                        TextInput::make('year')
                            ...
                            ->minValue(1900)
                            ->maxValue(2100),
                    ])->columns(4), // Places these 4 items nicely in a row


                // --- SECTION 2: LOCATION ---
                \Filament\Forms\Components\Fieldset::make('Location Details')
                    ->schema([
                        ...
                        Select::make('municipality')
                            ->label('Municipality')
                            ->options([
                                'Atok' => 'Atok', 'Bakun' => 'Bakun', 'Bokod' => 'Bokod', 'Buguias' => 'Buguias',
                                'Itogon' => 'Itogon', 'Kabayan' => 'Kabayan', 'Kapangan' => 'Kapangan',
                                'Kibungan' => 'Kibungan', 'La Trinidad' => 'La Trinidad', 'Mankayan' => 'Mankayan',
                                'Sablan' => 'Sablan', 'Tuba' => 'Tuba', 'Tublay' => 'Tublay',
                            ])
                            ->searchable()
                            ->required(),
                    ])->columns(2), // 2 items in a row


                // --- SECTION 3: CAPACITY & STAFFING ---
                \Filament\Forms\Components\Fieldset::make('Capacity & Staffing')
                    ->schema([
                        ...
                        TextInput::make('female_employees')
                            ...
                            ->numeric()
                            ->default(0),
                    ])->columns(3), // 3 items across


                // --- SECTION 4: GUEST ARRIVALS (GA) ---
                \Filament\Forms\Components\Fieldset::make('Guest Arrivals (GA)')
                    ->schema([
                        TextInput::make('ga_ph_province')
                            ->label('Philippine Residents')
                            ->helperText('What Province? (e.g., Benguet, Cebu)')
                            ->maxLength(255),

                        TextInput::make('ga_non_fil_country')
                            ->label('Foreign Residents')
                            ->helperText('What Country? (e.g., South Korea, Japan)')
                            ->maxLength(255),

                        TextInput::make('ga_overseas_filipinos')
                            ->label('Overseas Filipinos')
                            ->helperText('Filipinos born/raised in another country')
                            ->numeric()
                            ->default(0),

                        TextInput::make('ga_unspecified')
                            ->label('Unspecified Residents')
                            ->numeric()
                            ->default(0),
                    ])->columns(2),


                // --- SECTION 5: GUEST NIGHTS (GN) ---
                \Filament\Forms\Components\Fieldset::make('Guest Nights (GN)')
                    ->schema([
                        TextInput::make('gn_ph_province')
                            ->label('Philippine Residents (GN)')
                            ->helperText('Formula: GA x No. of Nights')
                            ->maxLength(255),

                        TextInput::make('gn_non_fil_country')
                            ->label('Foreign Residents (GN)')
                            ->helperText('Formula: GA x No. of Nights')
                            ->maxLength(255),

                        TextInput::make('gn_overseas_filipinos')
                            ->label('Overseas Filipinos (GN)')
                            ->numeric()
                            ->default(0),

                        TextInput::make('gn_unspecified')
                            ->label('Unspecified Residents (GN)')
                            ->numeric()
                            ->default(0),
                    ])->columns(2),


                // --- SECTION 6: ROOM OCCUPANCY ---
                \Filament\Forms\Components\Fieldset::make('Occupancy Data')
                    ->schema([
                        TextInput::make('rooms_occupied')
                            ->label('Total Rooms Occupied this Month')
                            ->numeric()
                            ->default(0)
                            ->columnSpanFull(),
                    ]),
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('ga_stats')
                    ...
                    ->getStateUsing(fn ($record) => 'PH Prov: ' . ($record->ga_ph_province ?: '0'))
                    ->description(fn ($record) => 'Foreign: ' . ($record->ga_non_fil_country ?: '0') . ' | Unspecified: ' . $record->ga_unspecified . ' | OF: ' . $record->ga_overseas_filipinos),

                TextColumn::make('gn_stats')
                    ->label('Guest Nights (GN)')
                    ->getStateUsing(fn ($record) => 'PH Prov: ' . ($record->gn_ph_province ?: '0'))
                    ->description(fn ($record) => 'Foreign: ' . ($record->gn_non_fil_country ?: '0') . ' | Unspecified: ' . $record->gn_unspecified . ' | OF: ' . $record->gn_overseas_filipinos),
